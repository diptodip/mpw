...
def residual_graph(G_, f):
    G = Graph(G_.V[:], dict(G_.E))
    for (u, v, x) in f.keys():
        if f[(u, v, x)] > 0 and G.E.has_key((u, v, x)):
            if G.E[(u, v, x)][0] >= 0:
                weight, capacity = G.E[(u, v, x)]
                capacity -= f[(u, v, x)]
                G.E[(u, v, x)] = (weight, capacity)
                G.E[(v, u, x)] = (0 - weight, f[(u, v, x)])
            else:
                weight, capacity = G.E[(u, v, x)]
                capacity = f[(u, v, x)]
                G.E[(u, v, x)] = (weight, capacity)
                weight, capacity = G.E[(v, u, x)]
                G.E[(v, u, x)] = (weight, capacity - f[(u, v, x)])
    return G

def djikstra_path(G, s, t, a, source, sink):
    ...
    for i in range(len(P[t]) - 1):
        u = P[t][i]
        v = P[t][i+1]
        if u is not sink and v is not source:
            path.append((u, v, 0))
        else:
            path.append((u, v, 1))
        if v is t:
...
def augment_flow(G, f, P):
    for e in P:
        if f.has_key(e) and f[e] < G.E[e][1]:
            f[e] += 1
...
def add_back_arcs(G, s, t):
    edges = [e for e in G.E.keys()]
    for e in edges:
        if e[0] is not s and G.E[e][0] >= 0:
            G.E[(e[1], e[0], 0)] = (0 - G.E[e][0], 0)
        elif e[0] is s and e[1] is t and G.E[e][0] >= 0:
            G.E[(e[1], e[0], 1)] = (0 - G.E[e][0], 0)
...
def minimum_energy_disjoint_paths(G, source_index, sink_index, k):
    ...
    while i <= len(sorted_neighbors):
        ...
        P, d_i_prime = djikstra_path(residual_G, sorted_neighbors[i-1], s, prev_a, s, t)
        print("path: " + str(P))
        for v in V:
        ...
        #print("d: " + str(d))
        #print("opt: " + str(opt))
        #print("current: " + str(neighbor_costs[sorted_neighbors[i-1]] + calc_flow_cost({e: f[e] for e in f.keys() if f[e] > 0}, residual_G.E)))
        if neighbor_costs[sorted_neighbors[i-1]] + calc_flow_cost({e: f[e] for e in f.keys() if f[e] > 0}, residual_G.E) <= min(opt, M):
            opt = neighbor_costs[sorted_neighbors[i-1]] + calc_flow_cost({e: f[e] for e in f.keys() if f[e] > 0}, residual_G.E)
            for e in G.E.keys():
